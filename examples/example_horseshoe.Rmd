...
Warning: This example workflow was only tested using Linux (Ubuntu). If you encounter any problems with the installation of cmdstan or cmdstanr, please use the example workflow using cmdstan directly (example_cmdstan.Rmd).
...
```{r}
read_counts_in <- fread(paste0(data_dir, "proQ_RIF-seq_1-100.csv"), data.table=F)
...
stan_file <- paste0(getwd(), "/../stan_models/LNM_horseshoe.stan")
...
fit_horse <- mod$sample(data = json_file, chains = 2, adapt_delta = 0.95, parallel_chains = 2, iter_warmup = 500, iter_sampling = 500, max_treedepth = 12)

# save fitted model
fit_horse$save_object(file = paste0(res_dir, "fit_horse-100.RDS"))
rm(fit_horse)

# cpu_time_horse <- system.time(mod$sample(data = json_file, chains = 2, adapt_delta = 0.95, parallel_chains = 2, iter_warmup = 500, iter_sampling = 500, max_treedepth = 12))
```
...
# save fitted model
fit0$save_object(file = paste0(res_dir, "fit_LNM-100.RDS"))
rm(fit0)

# cpu_time <- system.time(mod$sample(data = json_file, chains = 2, adapt_delta = 0.95, parallel_chains = 2, iter_warmup = 500, iter_sampling = 500, max_treedepth = 12))
```
...
```{r}
# load fitted model
fit_horse <- readRDS(paste0(res_dir, "fit_horse_02-100.RDS"))
fit0 <- readRDS(paste0(res_dir, "fit_LNM-100.RDS"))

# use the bayesplot packages to check for divergent transitions and correlations between parameters
# Pairs plots
x <- fit0$draws()
i <- "LNM_1"
p <- mcmc_pairs(x, pars = paste0("delta_beta[", 1:5, "]"),
...
```{r}
delta_beta_LNM <- fit0$draws(variables=c("delta_beta"), format="matrix") %>% apply(2, function(x) quantile(x, c(0.16, 0.5, 0.84)))
delta_beta_LNMhorse <- fit_horse$draws(variables=c("delta_beta"), format="matrix") %>% apply(2, function(x) quantile(x, c(0.16, 0.5, 0.84)))
beta_WT_LNM <- fit0$draws(variables=c("beta_WT"), format="matrix") %>% apply(2, median)
beta_WT_LNMhorse <- fit_horse$draws(variables=c("beta_WT"), format="matrix") %>% apply(2, median)
data.frame(delta_beta=c(delta_beta_LNM[2,]/beta_WT_LNM, delta_beta_LNMhorse[2,]/beta_WT_LNMhorse), model=rep(c("LNM", "LNMhorse"), each=N_g)) %>% ggplot() +
  geom_histogram(aes(delta_beta, col=model))
p_beta_LNM <- fit0$draws(variables=c("delta_beta"), format="matrix") %>% apply(2, function(x) p_value(x, int0=5e-2))
p_beta_LNMhorse <- fit_horse$draws(variables=c("delta_beta"), format="matrix") %>% apply(2, function(x) p_value(x, int0=5e-2))

compare_df <- data.frame(delta_beta_LNM=delta_beta_LNM[2,]/beta_WT_LNM,
           LNM_max=delta_beta_LNM[3,]/beta_WT_LNM,
           LNM_min=delta_beta_LNM[1,]/beta_WT_LNM, p_beta_LNM,
           delta_beta_LNMhorse=delta_beta_LNMhorse[2,]/beta_WT_LNMhorse,
           LNMhorse_max=delta_beta_LNMhorse[3,]/beta_WT_LNMhorse,
           LNMhorse_min=delta_beta_LNMhorse[1,]/beta_WT_LNMhorse, p_beta_LNMhorse, locus_tag=rownames(proQ_reads))
compare_df$com_id <- meta_data$com_id[match(compare_df$locus_tag, meta_data$locus_tag)]
compare_df %>% ggplot(aes(delta_beta_LNM, delta_beta_LNMhorse)) +
  geom_point() +
  geom_errorbar(aes(ymin=LNMhorse_min, ymax=LNMhorse_max)) +
  geom_errorbarh(aes(xmin=LNM_min, xmax=LNM_max)) +
  geom_text(aes(label=com_id), data=subset(compare_df, p_beta_LNM<=0.082 & p_beta_LNMhorse>0.082))

subset(compare_df, p_beta_LNM <= 0.082)
```

Extract parameters of interest and plot decay curves

```{r}
list_param_horse <- fit_horse$draws(format="matrix") %>% as.data.frame() %>% extract_params()
list_param_horse$locus_tag <- rownames(proQ_reads)
list_param_horse$com_id <- meta_data$com_id[match(list_param_horse$locus_tag, meta_data$locus_tag)]
```
...
```{r}
lab <- "yaeH"
ig <- which(list_param_horse$com_id==lab)
t <- (0:240)/10
fit_df <- data.frame(
  time=c(t,t),
  logrel=c(
    sapply(t, function(x) calc_mu(list_param_horse$pib_WT[ig], list_param_horse$beta_WT[ig], list_param_horse$gamma[ig], x, "LNM")),
    sapply(t, function(x) calc_mu(list_param_horse$pib_1[ig], list_param_horse$beta_1[ig], list_param_horse$gamma[ig], x, "LNM"))),
...
fit_df <- data.frame(
  ...
  condition=rep(c("WT", "dproQ"), each=length(t)))
p_decay <- subset(stan_df, com_id==lab) %>% ggplot(aes(x=time, y=logrel, col=condition)) +
  geom_point() +
  geom_line(aes(group=condition), data=fit_df) +
  scale_y_continuous(name="rel. log-abundance") +
  scale_color_manual(name="", values=cb_palette_4[c(1,2)]) +
  theme(axis.line=element_line(linewidth=unit(0.1, "pt")),
        legend.position="top", legend.box="vertical",
        legend.margin=margin(t=-0.15, b=-0.15, unit="cm"),
...
draws_df0 <- fit0$draws(variables=c(paste0("beta_WT[", ig, "]"), paste0("delta_beta[", ig, "]")), format="matrix") %>% as.data.frame()
draws_df_horse <- fit_horse$draws(variables=c(paste0("beta_WT[", ig, "]"), paste0("delta_beta[", ig, "]")), format="matrix") %>% as.data.frame()
plot_df <- data.frame(beta_WT=c(draws_df0[,1], draws_df_horse[,1]),
                      delta_beta=c(draws_df0[,2], draws_df_horse[,2]),
                      model=rep(c("LNM", "LNM_horse"), each=nrow(draws_df0)))
p_hist <- plot_df %>% ggplot() +
  geom_density(aes(beta_WT, col="WT", linetype=model), position="identity", fill="transparent") +
  geom_density(aes(beta_WT+delta_beta, col="dproQ", linetype=model), position="identity", fill="transparent") +
  scale_color_manual(name="", values=cb_palette_4[c(1,2)], breaks=c("WT", "dproQ")) +
  scale_linetype_manual(name="", values=c(1,2)) +
  scale_x_continuous(name=expression(paste(beta, " [1/min]"))) +
  theme(axis.line=element_line(linewidth=unit(0.1, "pt")),
        legend.position="top", legend.box="vertical",
        legend.margin=margin(t=-0.15, b=-0.15, unit="cm"),
        legend.key.width=unit(0.3,"cm"),legend.key.height=unit(0.3,"cm"),
...
p <- cowplot::plot_grid(p_decay, p_hist, ncol=2, labels=c('A', 'B'), rel_widths=c(0.4,0.4), label_size=14, scale=0.9)
print(p)
cairo_pdf(paste(res_dir, "summary_horse_", lab, ".pdf", sep=""), width = 6, height = 3)
print(p)
dev.off()
```

...
```{r}
# The functions used here are all highly customized. It is also straight forward to work directly with the draws from the Stan fit.
...
list_param_cdv <- fit_cdv$draws(format="matrix") %>% as.data.frame() %>% extract_params()
list_param_cdv$locus_tag <- rownames(proQ_reads)
list_param_cdv$com_id <- meta_data$com_id[match(list_param_cdv$locus_tag, meta_data$locus_tag)]
...
list_param_sigma <- list_param_cdv %>% pivot_longer(matches("sigma"), names_to="time", names_prefix="sigma_g", values_to="sigma")
...
Compare CPU time and parameters (hl_WT, delta_hl and p values) computed using the LNM, NB and LNMcdv

...
p <- cowplot::plot_grid(
  cowplot::plot_grid(p_pred_lnm, p_pred_cdv, labels=c('A', 'B', 'C'), label_size=14, ncol=3, scale=0.95),
  legend,
  ncol=1, rel_heights=c(0.5, 0.1), scale=1)
print(p)
cairo_pdf(paste(res_dir, "posterior_predictive.pdf", sep=""), width = 6.9, height = 2.4)
print(p)
dev.off()
```
...
p_loo <- stan_df %>% ggplot(aes(raw)) +
  geom_point(aes(y=elpd_nb, col=factor(time))) +
  scale_color_manual(name="time", values=c(viridis(5)[1:4], "orange")) +
  scale_alpha_continuous(name=expression(paste(t[1/2, WT], "/", CI["68%"]))) +
  ylab("ELPD") +
  scale_x_log10(name="raw count") +
  theme(legend.title=element_text(face="bold"),
...
p_loo <- stan_df %>% ggplot(aes(condition)) +
  ...
  geom_violin(aes(y=elpd_nb, col="NB")) +
  scale_color_manual(name="model", values=c(viridis(3)[1:2], "orange")) +
  scale_alpha_continuous(name=expression(paste(t[1/2, WT], "/", CI["68%"]))) +
  ylab("ELPD") +
  theme(legend.title=element_text(face="bold"),
...
p_loo_hist <- stan_df %>% ggplot() +
  ...
  geom_density(aes(x=elpd_nb, fill="NB")) +

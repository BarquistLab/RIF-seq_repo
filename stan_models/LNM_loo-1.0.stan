...
transformed data{
    ...
    array[N_g, 2*N_s] real X;
    array[N_g, 2*N_s + N_con + 4] int y;
    real t_min = min(t);
    vector[N_tot] t_rel = t - t_min;
...
model{
    ...
    
    {
      ...
      vector[0] param; // parameters shared by all genes
...
generated quantities{
  ...
  vector[N_tot] y_rep;
  {
    ...
    for (i in 1:N_tot) {
      ...
      y_rep[i] = normal_rng(log(pi_g), sigma_g[it_g[i],it_t[i]]);
    }
  }
}
